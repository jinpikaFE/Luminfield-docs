...
阶段 B 已在现有 192×128 大世界内加入首批村庄阵容与六个室内；阶段 F 第十九个纵切把完整关系 NPC 扩到 12 名，第二十个纵切补齐当时 12 人的关系内容，第二十一个纵切再把完整关系 NPC 扩到 16 名，达到阶段 F 人数下限：

- 村庄范围：`x=77..115`、`y=30..63`，通过晶辉大道与家园道路连通。
- 11 处外部地标：月辉档案馆、星织茶屋、月石工坊、星井广场、村门、路牌、双灯长椅、发光花车、暮光商铺、星光邮驿和星落巡守所。
- 16 名村民：莉欧拉 `liora`、塔维 `tavi`、奈米 `nemi`、塞菈 `sela`、艾洛温 `elowen`、维莎 `vessa`、奥林 `orin`、凯尔 `kael`、哈尔登 `halden`、玛维娅 `mavea`、西芙伦 `sivren`、多里克 `dorrik`、伊瓦拉 `yvara`、布里尔 `brial`、帕芙里 `pavri`、洛文 `roven`。
...
- 哈尔登、玛维娅、西芙伦和多里克各自拥有完整普通/灯憩/天气或季节路线、三个既有主节日的唯一锚点和双语对白，以及两段 25/60、每段 3 页的关系事件；第二段只能在第一段更早日完成后触发。
- 艾洛温拥有“井沿的潮痕”与“共读水位线”，维莎拥有“苦叶与暖杯”与“会回应的叶径”；四段事件都从真实日程投影、真实邻接和“手”工具触发。凯尔、塞菈、艾洛温、维莎与奥林的第二段事件还会在次日投递一次性关系回信。
- 四名新人第二段事件完成后的下一日各投递一次奖励星邮；投递规则读取事件完成日，不会仅凭 60 点关系提前发放。
- 伊瓦拉、布里尔、帕芙里和洛文各自拥有完整普通/灯憩/天气或季节路线、三个既有主节日的唯一锚点和双语对白，以及两段 25/60、每段 3 页的关系事件；第二段完成后的下一日分别投递月梅树苗、星蜜、月绒和星光火炬奖励星邮。

## 状态与数据

- `VillageCatalog` 保存稳定人物 ID、地标 ID、村庄边界、道路、碰撞与日程定义。
- `NpcScheduleEntry` 使用开始/结束分钟、稳定场景 ID、目标位置、朝向、本地化键、星期、天气、季节和显式优先级。
- `VillageSystem` 保存已认识人物与关系记录，并把同一个 `WeatherSystem.CurrentId` 交给 `NpcScheduleSystem`；后者按日期、分钟、条件与玩家当前格批量解析 16 人投影。
...
- `MailSystem` 保存稳定邮件 ID、送达日、已读与附件领取状态；数据化投递规则支持相遇、关系阶段或角色事件完成，事件奖励要求完成日早于当前日。
- `CharacterEventSystem` 保存稳定事件 ID 与完成日期；16 条事件链分别归一化，目录同时验证人物、前置存在、同人、定义顺序、递增阈值和互异对白页，资格由交互前的相遇、关系、场景、真实邻接、工具、日程对话键和前置事件状态纯判定；已有活动事件不能被另一事件覆盖。
...
- `assets/generated/village_npcs_wave_2_chroma.png` / `village_npcs_wave_2.png`：严格 4×4，四行依次为哈尔登、玛维娅、西芙伦和多里克，四列为下/上/左/右；1024×1024，二值 alpha、透明四角、0 个近品红残留，16 帧基线统一。
- `assets/generated/village_npcs_wave_3_chroma.png` / `village_npcs_wave_3.png`：严格 4×4，四行依次为伊瓦拉、布里尔、帕芙里和洛文，四列为下/上/左/右；1024×1024，二值 alpha、透明四角、0 个近品红残留，16 帧底基线统一为单元内 `y=238`。
- `NpcArtCatalog` 按稳定人物 ID 映射 `village_npcs_base`、`village_npcs_expansion`、`village_npcs_wave_2` 与 `village_npcs_wave_3` 四套图集；日程排序与图集行号分离，未知人物/图集或越界行明确失败，不再夹取最后一行。
...
- 第一版关系奖励、星邮、暮光商铺轮换库存，以及当前 16/16 名村民各自的两阶段友情事件和至少一封关系奖励邮件已完成；阶段 F 的 16–20 名关系内容目标已达到最低 16 人口径，但尚未扩展到可选上限 20 人。

## 验证

- 当前全量测试 468/468 通过；中英本地化键集合为 1376/1376。
- macOS 已实际查看暮光商铺轮换库存与灯憩日关闭，以及奥林两阶段事件的普通日下午真实广场投影；截图 99–100、104–105 来自第 8 轮集成态。
- 1–56 日、四种显式天气、全天每 10 分钟解析 16 名跨场景互不重叠、对应区域可通行的村民位置；同场景相邻刻度最多移动一格且不发生对穿。
- 灯憩日路线与普通日不同。
...
- macOS 已在 Apple M5 / Metal 实际查看四名新人世界/档案馆投影、英文对话、暖金错误工具提示和三个主节日的 12 人布局；截图为 280–286。
- macOS 已在 Apple M5 / Metal 实际查看艾洛温与维莎四段事件、维莎真实人物目标的暖金错误工具态，以及五封英文事件后回信与附件；截图为 290–295。
- macOS 已在 Apple M5 / Metal 实际查看伊瓦拉、布里尔、帕芙里和洛文的世界/室内投影、英文对白、暖金错误工具态、伊瓦拉两阶段事件与四封英文关系回信；截图为 300–306。星熟集市、芽辉播种祭和长夜灯宴的 16 人布局、设施与出口回归截图为 307–309。
